...
| **Fase 5 lista (25/09): PAT ampliado + `scripts/gh-publish.ps1`** | Reutilizar el PAT fine-grained existente (agregar repo + *Update token*, valor intacto) en vez de un PAT por repo (un solo `GH_TOKEN` global no admite dos valores); script copiado del template sin el mensaje de Pages; verificación solo-lectura por decisión del usuario | `scripts/gh-publish.ps1`; `gh auth status` OK (`seiji142`); `gh pr list` exit 0 tras el Paso 0; protección vía CLI en 403 a propósito (exige Administration read, omitido; ya verificada anónimo+UI) |
