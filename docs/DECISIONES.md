...
| **Fase 5 completa (25/09): PAT ampliado + `scripts/gh-publish.ps1` + PR #1** | Reutilizar el PAT fine-grained existente (agregar repo + *Update token*, valor intacto) en vez de un PAT por repo (un solo `GH_TOKEN` global no admite dos valores); script copiado del template sin el mensaje de Pages; escritura probada real con PR #1 (`develop → main`, 5 commits, MERGEABLE, sin mergear) | `scripts/gh-publish.ps1`; https://github.com/seiji142/youtube-transcripts/pull/1; protección vía CLI en 403 a propósito (exige Administration read, omitido; ya verificada anónimo+UI) |
| **Template migrado a repo GitHub `seiji142/gitflow-scaffold` (25/09), versión `2026.09.25`** | Fuente de verdad única = repo privado (deja de ser la carpeta local `Proyecto AI/templates/gitflow-scaffold`, que solo retiene el espejo del script). Adoptado: `gh-publish.ps1` con `-Base`, validación pre-PR obligatoria (variante B: pytest + criterio explícito del usuario, sin excepciones), sección Publicación (nunca `gh pr create/merge` directos) y `ci.yml` adaptado a Python; check `build` requerido en `main` pendiente de activación manual en UI (PAT sin Administration → 403, nota del propio template) | `.ai/commands.md`, `.github/workflows/ci.yml`, `scripts/gh-publish.ps1` (90 líneas, con `-Base`); `gitflow-scaffold.md` § header + checklist §7 |
